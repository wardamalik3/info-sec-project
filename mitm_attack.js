...
// Alice
const { publicKey: alicePub, privateKey: alicePriv } = require('crypto').generateKeyPairSync('ec', { namedCurve: 'P-256' });
// Mallory
const { publicKey: malloryPub, privateKey: malloryPriv } = require('crypto').generateKeyPairSync('ec', { namedCurve: 'P-256' });

// 2. Alice generates Ephemeral Key and Signs it
console.log('\n2. Alice generates Ephemeral Key and Signs it...');
const aliceEphemeral = require('crypto').generateKeyPairSync('ec', { namedCurve: 'P-256' });
const alicePayload = {
    ephemeralKey: aliceEphemeral.publicKey.export({ type: 'spki', format: 'pem' }),
    timestamp: Date.now()
};
...
console.log('\n3. Mallory Intercepts and Modifies Payload...');
const malloryEphemeral = require('crypto').generateKeyPairSync('ec', { namedCurve: 'P-256' });
const malloryPayload = {
    ephemeralKey: malloryEphemeral.publicKey.export({ type: 'spki', format: 'pem' }), // Replaced Key!
    timestamp: Date.now()
};
...
if (isValid2) {
    console.log('MITM SUCCESS: Signature verified! (Impossible unless Mallory has Alice\'s private key)');
} else {
    console.log('MITM FAILED: Signature verification failed! Bob used Alice\'s Public Key to verify.');
}
